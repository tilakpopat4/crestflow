...
export default function IntroScreen({ onComplete }: IntroScreenProps) {
  ...
  return (
    <div
      ...
      className="fixed inset-0 bg-slate-950 flex flex-col justify-between p-6 md:p-12 z-[100] text-white select-none overflow-hidden font-sans"
    >
      {/* Safe zone guides (Video Editor Overlay) */}
      <div className="absolute inset-8 border border-slate-900 pointer-events-none z-10 flex flex-col justify-between p-2">
      ...
      <div className="flex justify-between items-center z-20">
        ...
        <button
          onClick={skipIntro}
          className="px-4 py-1.5 bg-slate-900/60 hover:bg-indigo-900/40 border border-slate-800 hover:border-indigo-500/50 rounded-xl text-xs font-semibold tracking-wider uppercase text-slate-400 hover:text-white transition-all duration-200"
        >
          Skip Intro
        </button>
      </div>
      ...
      >
        ...
        <div className="absolute inset-2 rounded-full border border-dashed border-purple-500/30"></div>
        <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-indigo-500/30"></div>
        <div className="absolute left-1/2 top-0 bottom-0 w-[1px] bg-indigo-500/30"></div>
        <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] font-mono text-indigo-400">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-8 z-20 relative">
        ...
        >
          <svg width="120" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="filter drop-shadow-lg">
            {/* Hinge Joint */}
            <circle cx="20" cy="55" r="4" fill="#a5b4fc" />
            ...
            >
              <rect x="15" y="38" width="90" height="12" rx="3" fill="#1e293b" stroke="#f1f5f9" strokeWidth="2" />
              <path d="M30 38 L42 50 M50 38 L62 50 M70 38 L82 50 M90 38 L102 50" stroke="#f1f5f9" strokeWidth="3" />
            </motion.g>

            {/* Clapperboard Base */}
            <g>
              <rect x="15" y="55" width="90" height="35" rx="4" fill="#0f172a" stroke="#f1f5f9" strokeWidth="2" />
              <path d="M15 55 L25 65 M35 55 L45 65 M55 55 L65 65 M75 55 L85 65" stroke="#f1f5f9" strokeWidth="3" />
              <text x="32" y="77" fill="#818cf8" fontSize="6" fontWeight="bold" fontFamily="monospace">SCENE</text>
              <text x="36" y="86" fill="#f1f5f9" fontSize="8" fontWeight="bold" fontFamily="monospace">01</text>
              <text x="62" y="77" fill="#818cf8" fontSize="6" fontWeight="bold" fontFamily="monospace">TAKE</text>
          ...
          <AnimatePresence>
            {clapped && (
              <motion.div
                ...
                className="absolute inset-0 rounded-full border border-indigo-500/80 pointer-events-none"
              ></motion.div>
            )}
          </AnimatePresence>
        </motion.div>
        ...
        <div className="flex flex-col items-center text-center">
          <div className="flex overflow-hidden">
            {"CRESTFLOW".split("").map((letter, index) => (
              <motion.span
                ...
                className="font-display font-extrabold text-5xl md:text-7xl tracking-wider bg-gradient-to-r from-white via-indigo-200 to-indigo-500 bg-clip-text text-transparent"
              >
                {letter}
              </motion.span>
            ))}
          ...
          <motion.p
            ...
            className="text-xs md:text-sm font-medium uppercase tracking-widest text-slate-400 mt-2 font-display flex items-center gap-1.5"
          >
            Creative Workspace
      ...
      <div className="w-full max-w-4xl mx-auto space-y-6 z-20">
        ...
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-4 space-y-3 backdrop-blur-xs relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono text-slate-500 w-6 flex-shrink-0 flex items-center gap-1">
              <Film className="w-3 h-3 text-slate-400" />
              V1
            </span>
            <div className="flex-1 bg-slate-950/60 rounded-md h-8 relative overflow-hidden border border-slate-900 flex items-center p-1 gap-2">
              <div className="w-1/3 bg-indigo-900/40 border border-indigo-800/60 rounded h-full flex items-center px-2 text-[9px] font-mono text-indigo-200">
                A_Roll_01.mp4
              </div>
              <div className="w-1/4 bg-indigo-500/20 border border-indigo-500/30 rounded h-full flex items-center px-2 text-[9px] font-mono text-indigo-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 bg-slate-950/60 rounded-md h-8 relative overflow-hidden border border-slate-900 flex items-center p-1 gap-2">
              <div className="w-full bg-emerald-950/30 border border-emerald-900/40 rounded h-full flex items-center px-2 justify-between">
                <span className="text-[9px] font-mono text-emerald-300">Background_Synth.wav</span>
        ...
        <div className="space-y-2">
          ...
          {/* Progress bar track */}
          <div className="w-full bg-slate-900 border border-slate-800 rounded-full h-2 relative overflow-hidden">
            <motion.div 
