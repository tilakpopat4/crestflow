  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">Work Log</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Log completed edits and services before generating invoices.</p>
        </div>
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative flex-1 md:w-64">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-800 text-xs pl-9 pr-7 py-2 rounded-lg border border-slate-200 dark:border-slate-700 focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-1 focus:ring-indigo-500 outline-none text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
            {searchQuery && (
              <button
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 px-3 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-transparent dark:border-slate-700 rounded-lg text-xs font-semibold transition-colors cursor-pointer shrink-0"
      ...
      {isFormOpen && (
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 mb-8">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">{editingWorkId ? 'Edit Work Log Entry' : 'Log Completed Work'}</h3>
            <button onClick={handleCancel} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer">×</button>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {(() => {
                  ...
                  if (selClient?.subClients && selClient.subClients.length > 0) {
                    return (
                      <div className="mt-2.5 bg-purple-50 dark:bg-purple-950/30 p-2.5 rounded-lg border border-purple-200 dark:border-purple-800/60">
                        <label className="block text-xs font-bold text-purple-900 dark:text-purple-300 mb-1 flex items-center gap-1">
                          <Users size={12} className="text-purple-600 dark:text-purple-400" /> Assign Sub-Client (Optional)
                        </label>
                        <select
                          className="w-full p-2 border border-purple-300 dark:border-purple-700 rounded-md text-xs bg-white dark:bg-slate-800 font-medium text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-purple-500"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Description (e.g., Real Estate Reel, VLOG Edit)</label>
              <input 
                type="text"
                required
                className="w-full p-2.5 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
            ...
            <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-700">
              <button 
                ...
                className="px-4 py-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded font-medium mr-2 cursor-pointer"
              >
                Cancel
              </button>
              <button 
                type="submit" 
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded font-medium transition-colors cursor-pointer"
              >
      ...
      {sortedWork.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
          ...
          <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100 mb-1">No work logged yet</h3>
          <p className="text-slate-500 dark:text-slate-400">Start logging your edits and services to easily generate invoices later.</p>
        </div>
      ) : (
                    ...
                    <tr key={work.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="py-4 px-4 text-sm text-right flex justify-end items-center gap-1">
                        ...
                        <button 
                          ...
                          className="text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 p-1.5 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
                          title="Edit Work Log"
                        >
                          <Edit2 size={16} />
                        </button>
                        <button 
                          ...
                          className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-950/50 transition-colors cursor-pointer"
